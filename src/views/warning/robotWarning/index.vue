<template>
  <div class="robot-warning app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true">
      ...
      <el-form-item label="处理状态" prop="warningState">
        <el-select
        ...
        >
          <el-option
            v-for="dict in warningStateOption"
            :key="dict.dictValue"
    ...
    >
      ...
      />
      <el-table-column label="结果" align="center" prop="warningInfo" />
      <el-table-column
        label="操作"
        align="center"
  ...
  methods: {
    loadDicts() {
      this.getDicts("robot_warning_status").then(response => {
        this.warningStateOption = response.data;
      });
    },
    async loadUnHandledList() {
      this.loading = true;
      const res = await getWarningRobotList(this.queryParams);
    ...
    async handleDelete(row) {
      ...
      console.log(Ids);
      await getWarningRobotDelete(Ids);
